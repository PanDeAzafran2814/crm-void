...
import { RenewalSuscription } from "@/app/lib/suscriptions/actions";
...
export default function RenwalSuscriptionForm({
    suscripcionData
}:{
    ...
    const RenewalSuscriptionWithId = RenewalSuscription.bind(null, suscripcionData.id)
    const [state, dispatch] = useFormState(RenewalSuscriptionWithId, initialState)
    ...
    const [form_date, setform_date] = useState(suscripcionData.renewal_date ? new Date(suscripcionData.renewal_date).toISOString().split('T')[0] : "");
